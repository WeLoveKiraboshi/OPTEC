function f = check_obstacle_pos(center, r, C, R, Rect, A, B, n_discretization)
...
for i=1:n
    if norm(C(i, :)-center) == 0 && R(i) == r %skip itself if exists
        continue;
    end
...
for k=0:n_walls-1
    ...
    if P1(1) <= center(1) && center(1) <= P2(1) && P1(2) <= center(2) && center(2) <= P4(2)
        f = f * false;
    end
    f = f * ((r - distance_segment_circle(P1, P2, center, n_discretization, false)) < 0);
    f = f * ((r - distance_segment_circle(P2, P3, center, n_discretization, false)) < 0);
    f = f * ((r - distance_segment_circle(P3, P4, center, n_discretization, false)) < 0);
    f = f * ((r - distance_segment_circle(P4, P1, center, n_discretization, false)) < 0);
end
